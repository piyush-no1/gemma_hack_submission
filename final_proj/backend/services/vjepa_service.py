...
from transformers import AutoProcessor, AutoModel, logging as transformers_logging
# Silence verbose transformers load reports (unexpected weight keys, etc.)
...
class VJepaAnomalyService:
    ...
    def _load_video(self, video_path: str, stride: int = 2) -> Tuple[List[np.ndarray], float]:
        ...
        # If too few frames, try loading with stride = 1
        if len(frames) < 16 and stride > 1:
            print(f"[V-JEPA] Video yielded only {len(frames)} frames with stride={stride}. Re-loading with stride=1...")
            return self._load_video(video_path, stride=1)
            
        # If still too few frames, pad by duplicating the last frame
        if len(frames) < 16 and len(frames) > 0:
            print(f"[V-JEPA] Padding video from {len(frames)} to 16 frames.")
            while len(frames) < 16:
