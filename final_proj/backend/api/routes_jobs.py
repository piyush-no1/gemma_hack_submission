...
async def stop_job(job_id: str):
    ...
    if not success:
        # Check if job exists in memory at all to give a better error message
        job = job_manager.get_job(job_id)
        if not job:
            raise HTTPException(status_code=404, detail="Job not found.")
        raise HTTPException(status_code=400, detail="Job cannot be stopped (either not running or already finished).")
