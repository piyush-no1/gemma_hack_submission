...
async def gemma_output_node(state: PipelineState) -> dict:
    ...
    # --- Safety override: parse final verdict string for emergency keywords ---
    emergency_keywords = ["fire", "smoke", "robbery", "altercation", "assault", "fight", "medical", "accident", "crash", "anomaly", "violence", "threat", "intrusion", "danger", "emergency", "abrupt", "blast", "explosion"]
    verdict_lower = judge_verdict.lower()
    if any(kw in verdict_lower for kw in emergency_keywords):
        print(f"[Node: Gemma Output] Safety override: detected emergency keyword in verdict '{judge_verdict}'. Setting judge_is_emergency=True.")
        judge_is_emergency = True
...
Generate the final emergency report in language '{output_language}'."""
    ...
    try:
        ...
        # Compile a visual summary from the frame transcripts to describe the video events
        transcript_descriptions = [
            t.description for t in transcript 
            if t.description and "[Visual transcription unavailable]" not in t.description
        ]
        if transcript_descriptions:
            explanation = "Visual analysis summary: " + " ".join(transcript_descriptions[:2])
            if len(transcript_descriptions) > 2:
                explanation += " " + transcript_descriptions[-1]
        else:
